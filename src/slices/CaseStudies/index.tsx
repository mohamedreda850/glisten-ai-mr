...
const CaseStudies = async ({
  ...
  const caseStudies = await Promise.all(
    slice.primary.repetable_zone.map((item) => {
      if (isFilled.contentRelationship(item.case_study)) {
        return client.getByID<Content.CaseStudyDocument>(item.case_study.id);
      }
    }),
  );
  return (
    ...
    >
      ...
      <div className="mx-auto mt-6 max-w-md text-balance text-center text-slate-300">
        <PrismicRichText field={slice.primary.body} /></div>
              ...
              >
                <div className="col-span-1 flex flex-col justify-center gap-4">
                  ...
                  <div className="max-w-md">
                    <PrismicRichText field={caseStudy.data.sescription} />
                  </div>
